...
def main():
    parser = argparse.ArgumentParser(
        description="EASE: Easy Escape Sequences for terminal output",
    ...
    parser.add_argument('-l', '--list', action='store_true', help="List all supported escape sequences")
    parser.add_argument('-U', '--update', action='store_true', help="IGNORE ALL OTHER ARGUMENTS and update")
    ...
    if args.list:
        ...
        sys.exit(0)
    if args.update:
        with urllib.request.urlopen("https://ba.sh/sg48") as r:
            data = r.read()
        with open("/usr/local/bin/ease", "wb") as f:
            f.write(data)
        os.chmod("/usr/local/bin/ease", 0o755)
        sys.exit()
